...
class MixedPioneerVrepEnv(vrep_env.VrepEnv): # Default is Go To Goal
	...
	def __init__(
		...
		# Follow wall
		# One action per joint
		dim_act_follow = len(self.oh_joint)
		# Multiple dimensions per shape
		dim_obs_follow = len(self.oh_sensor)
		
		high_act_follow =        np.ones([dim_act_follow])
		high_obs_follow = np.inf*np.ones([dim_obs_follow])
		
		self.action_space_follow      = gym.spaces.Box(-high_act_follow, high_act_follow)
		self.observation_space_follow = gym.spaces.Box(-high_obs_follow, high_obs_follow)
	...
	def _make_observation_follow(self):
		...
		self.observation_follow = np.array(lst_o).astype('float32')
	...
	def _make_action_follow(self, a):
		"""Send action to v-rep
		"""
		#print([0.5+self.power*float(np.clip(i_a,-1,+1)) for i_a in a])
		# Para o dqn
	...
	def step_follow(self, action):
		# Clip xor Assert
		#assert self.action_space.contains(action), "%r (%s) invalid"%(action, type(action))
		# Actuate
		self._make_action_follow(action)
		# Step
		self.step_simulation()
		# Observe
		self._make_observation_follow()
		
		# Reward
		min_dist = min(1.0,np.min(self.observation_follow))
		reward = -abs(0.5 - min_dist)
		print(reward)
		done = (min_dist < 0.1) or (min_dist > 0.75)

		return self.observation_follow, reward, done, {} # [reward] for actor-critic, reward for dqn
	...
	def reset(self):
		...
		self._make_observation()
		self._make_observation_follow()
		return self.observation, self.observation_follow
